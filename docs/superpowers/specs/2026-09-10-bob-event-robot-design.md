...
| 2× Waveshare ESP32-S3-Touch-LCD-2.1 (round 2.1", the boards actually on hand; the 1.85 was the original candidate) | ST7701 480×480 over 16-bit RGB parallel (DE 40, VSYNC 39, HSYNC 38, PCLK 41, 16 MHz), ST7701 init over 3-wire SPI (SCL 2, SDA 1, CS on TCA9554 EXIO3), LCD reset on TCA9554 EXIO1 (I2C SDA 15 / SCL 7, addr 0x20), backlight PWM GPIO6. Native USB on the port labelled "USB" (`/dev/cu.usbmodem*`; the "UART" port is a CH343), 16 MB QIO flash, 8 MB OPI PSRAM. Arduino_GFX `Arduino_ESP32RGBPanel` supports it; PlatformIO via pioarduino. | Firmware built here with PlatformIO, flashed with one script. Serial JSON protocol from the Jetson; each eye is told which side it is. |
